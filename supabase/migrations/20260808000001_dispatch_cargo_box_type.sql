-- 배차 등록/수정 시 선택하는 "적재함 종류"(cargo_box_type) 필드 추가.
-- 기존 데이터와 미입력을 허용해야 하므로 nullable, 값 검증은 check 제약으로 한다.
alter table public.dispatches
  add column cargo_box_type text;

alter table public.dispatches
  add constraint dispatches_cargo_box_type_check
  check (
    cargo_box_type is null
    or cargo_box_type in ('CARGO', 'BOX', 'WING', 'REFRIGERATED', 'OTHER')
  );

-- 아래 drop 시그니처는 적용 시점의 pg_proc 실제 identity arguments와 일치시킨 것이다.
-- (하나라도 다르면 구버전이 삭제되지 않고 오버로드로 남는다 —
--  20260806000004_cleanup_dispatch_rpc_overload.sql 참고)
drop function if exists public.create_dispatch(
  date, uuid, text, text, text, integer, numeric, uuid, text, text, text, text, text,
  text, text, text, bigint, boolean, bigint, text, text
);
