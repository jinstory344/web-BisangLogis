-- 매출 등록 폼의 "출처"(고객 유입 경로) 대분류/소분류 필드 추가.
alter table public.dispatches
  add column source_major text
    check (
      source_major is null
      or source_major in ('거래처', '지인', '카카오톡', '밴드', '24시콜화물', '기타')
    ),
  add column source_minor text;
...
begin
  ...
  insert into public.dispatches (
    ...
    carrier_name, contact_name, source_major, source_minor,
    ...
    p_carrier_name, p_contact_name, p_source_major, p_source_minor,
