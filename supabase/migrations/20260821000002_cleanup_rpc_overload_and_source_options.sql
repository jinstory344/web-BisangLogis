-- 지난 마이그레이션들이 cargo_box_type 포함 여부를 놓쳐서 create_dispatch/
-- update_dispatch에 각각 두 개의 오버로드가 남아있었다. cargo_box_type이
-- 없는(내가 실수로 만든) 오버로드를 지우고, 실제 쓰이는(cargo_box_type
-- 포함) 오버로드에 출처 대분류 확장 목록을 반영한다.
